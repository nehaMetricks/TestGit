 ...
 * 
 */
public class IncomingTrackerTrendBasedOnCategoryComputeInitialAndEndCount {
	...
	private static void queryResult(Statement stmt,
		...
		for (from_date = inputFromDate; from_date <= inputToDate; from_date++) {
			ResultSet query_set = stmt
					.executeQuery("select sum(newcount) count from (Select b.insert_date,(b.New_Count-a.new_count) newcount From (Select  New_Count,insert_date "
							+ "From Trend "
