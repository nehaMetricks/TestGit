 ...
 * 
 */
public class JSSTrackerTrendFromTrendPlotData3 {
	...
	private static DefaultCategoryDataset queryResult(
		...
		int init_date = Integer.parseInt(from_date);
		int final_date = Integer.parseInt(to_date);
		int next_date =final_date+1;
		...
		stmt = conn
				.prepareStatement("select new_count count, extract(month from insert_date) insert_month, extract(year from insert_date) insert_year "
						...
						+ "Where Extract(Year From Insert_Date) between ? and ?"
						+ "Or (Extract(Year From Insert_Date)= ? And  Extract(Month From Insert_Date)=1)"
						+ "Order By insert_date");

		stmt.setInt(1, init_date);
		stmt.setInt(2, final_date);
		stmt.setInt(3, next_date);
		...
		for (Map.Entry<Integer, Integer> entry : trendMap.entrySet()) {
			...
			if (year >= init_date && year <= final_date)
