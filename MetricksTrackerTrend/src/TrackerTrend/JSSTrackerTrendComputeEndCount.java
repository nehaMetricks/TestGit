 ...
 * 
 */
public class JSSTrackerTrendComputeEndCount {
	...
	public static void main(String[] args) throws Exception {
		...
		try {
			...
			queryResult(stmt, my_bar_chart_dataset, from_date, to_date);
			...
			// String category_name = getCategoryName(conn)
			// .toUpperCase();
	...
	private static void queryResult(Statement stmt,
			DefaultCategoryDataset my_bar_chart_dataset, String fromdate,
			String todate) throws SQLException, ClassNotFoundException {
		...
		for (from_date = inputFromDate; from_date <= inputToDate; from_date++) {
			...
			ResultSet query_set = stmt
					.executeQuery("select min(extract(year from insert_date)) ins_date, sum(newcount) count"
							+ " from "
//							+ "(Select b.insert_date, b.ncb-a.nca newcount"
//							+ " From (Select insert_date, sum( New_Count) nca"
//							+ " From Trend"
//							+ " Where Insert_Date In"
//							+ " (Select Max(Insert_Date) From Trend Where extract(year from insert_date) ='"
//							+ (from_date - 1)
//							+ "') group by insert_date) A,"
//							+ " (Select insert_date,sum(New_Count)ncb "
//							+ " From Trend"
//							+ " Where Insert_Date In"
//							+ " (Select Min(Insert_Date) From Trend Where extract(year from insert_date) ='"
//							+ from_date
//							+ " ') group by insert_date ) B "
//							+ " union "
							+ " (Select insert_date,Sum(New_Count)-(Lag(Sum(New_Count)) Over (Order By insert_date )) newCount "
							+ " From Trend  "
							+ " Where Insert_Date Between (Select Min(Insert_Date) From Trend Where Extract(Year From Insert_Date) = '"
							+ from_date
							+ " ') "
							+ " And (Select Min(Insert_Date) From Trend Where Extract(Year From Insert_Date) = '"
							+ (from_date + 1)
							+ " ') "
							+ " Group By insert_date) where newcount is not null");

			while (query_set.next()) {

				int count = query_set.getInt("count");

				// String date = query_set.getString("ins_date");
	...
	/*
	 * select min(extract(year from insert_date)),sum(newcount) from (Select
	 * b.insert_date, b.ncb-a.nca newcount From (Select insert_date, sum(
	 * New_Count) nca From Trend Where Insert_Date In (Select Max(Insert_Date)
	 * From Trend Where extract(year from insert_date) ='2012') group by
	 * insert_date) A, (Select insert_date,sum(New_Count)ncb From Trend Where
	 * Insert_Date In (Select Min(Insert_Date) From Trend Where extract(year
	 * from insert_date) ='2013')group by insert_date ) B union Select
	 * insert_date,Sum(New_Count)-(Lag(Sum(New_Count)) Over (Order By
	 * insert_date )) newCount From Trend Where Insert_Date Between (Select
	 * Min(Insert_Date) From Trend Where Extract(Year From Insert_Date) =
	 * '2013') And (Select Min(Insert_Date) From Trend Where Extract(Year From
	 * Insert_Date) = '2014') Group By insert_date)where newcount is not null ;
	 */
}