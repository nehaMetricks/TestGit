 ...
 * 
 */
public class TrendBasedOnCategorySaturday {
	public static void main(String[] args) throws Exception {
		...
		try {
			...
			Color c11 = Color.decode("0xE6E6D8");
			BarChartObject.setBackgroundPaint(c11);

			// Set font type of the chart title
			BarChartObject.getTitle().setFont(
					new Font("Calibri (Body)", Font.PLAIN, 10));
			...
			File BarChart = new File(
					"ChartForTrackerTrendBasedOnCategoryTrendTablesaturday.png");
	...
	private static DefaultCategoryDataset queryResult(Statement stmt,
		...
		ResultSet query_set = stmt
				.executeQuery("SELECT EXTRACT(YEAR FROM a.insert_date) \"year\", "
						+ "  a.insert_date, "
						+ "  b.new_count   - a.new_count \"newTrend\", "
						+ "  b.closed_count-a.closed_count \"closedTrend\", "
						+ "  a.total_pending \"pendingTrend\" "
						+ "FROM trend a "
						+ "INNER JOIN trend b "
						+ "ON a.id+1          = b.id "
						+ "WHERE a.category_id= "
		...
		while (query_set.next()) {
			...
			int newcount = query_set.getInt("newTrend");
			int closedcount = query_set.getInt("closedTrend");
	...
	private static JFreeChart createChart(
		...
		// Code to change the color of total_pending line chart

		Color c3 = Color.decode("0x000080");
		lineandshaperenderer.setSeriesPaint(0, c3);
