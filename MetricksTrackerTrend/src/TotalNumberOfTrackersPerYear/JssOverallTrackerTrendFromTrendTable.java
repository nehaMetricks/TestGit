 ...
 * 
 */
public class JssOverallTrackerTrendFromTrendTable {
	public static void main(String[] args) throws Exception {
		...
		try {
			...
			my_bar_chart_dataset = queryResult(stmt, from_date, to_date,
					"New_Count", my_bar_chart_dataset, "New Open");

			my_bar_chart_dataset = queryResult(stmt, from_date, to_date,
					"Closed_Count", my_bar_chart_dataset, "Closed");

			pendingTrackersdataset = queryResult(stmt, from_date, to_date,
					"Total_Pending", pendingTrackersdataset, "Pending");
			...
			/*
			 * ChartFactory.createBarChart("Trackers", null, null,
			 * my_bar_chart_dataset, PlotOrientation.VERTICAL, true, true,
			 * false);
			 */
	...
	private static DefaultCategoryDataset queryResult(Statement stmt,
			...
			DefaultCategoryDataset my_bar_chart_dataset, String series)
			throws SQLException {
		ResultSet query_set = stmt.executeQuery("Select Insert_Date, " + status
				+ " totalCount From Trend Where Insert_Date Between '"
				+ from_date + "' And '" + to_date + "' Group By insert_Date, "
				+ status + " order by insert_date");
